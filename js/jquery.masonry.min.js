...
(function(a, b, c) {
	...
	(d.special.smartresize = {
		...
		_create: function(c) {
			...
			this.element.css(e), (this.horizontalDirection = this.options.isRTL
				? "right"
				: "left"), (this.offset = {
				x: parseInt(
		...
		_getColumns: function() {
			...
			(this.columnWidth = this.isFluid
				...
				: this.options.columnWidth ||
						this.$bricks.outerWidth(!0) ||
						b), (this.columnWidth += this.options.gutterWidth), (this.cols = Math.floor(
				(b + this.options.gutterWidth) / this.columnWidth
			)), (this.cols = Math.max(this.cols, 1));
		},
		_placeBrick: function(a) {
			var c = b(a), d, e, f, g, h;
			(d = Math.ceil(c.outerWidth(!0) / this.columnWidth)), (d = Math.min(
				d,
				this.cols
			));
			if (d === 1) f = this.colYs;
				...
				(e = this.cols + 1 - d), (f = []);
				for (h = 0; h < e; h++)
					(g = this.colYs.slice(h, h + d)), (f[h] = Math.max.apply(
						Math,
						g
					));
			}
			...
			(m[this.horizontalDirection] =
				this.columnWidth * j + this.offset.x), this.styleQueue.push({
				$el: c,
				style: m
			});
		...
		resize: function() {
			var a = this.cols;
			this._getColumns(), (this.isFluid || this.cols !== a) &&
				this._reLayout();
		},
		...
		destroy: function() {
			...
			this.element
				.unbind(".masonry")
				.removeClass("masonry")
				.removeData("masonry"), b(a).unbind(".masonry");
		}
