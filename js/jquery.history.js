window.JSON || (window.JSON = {}), (function() {
	...
	(c.Adapter = {
		...
		extractEventData: function(a, c, d) {
			var e =
				(c && c.originalEvent && c.originalEvent[a]) ||
				(d && d[a]) ||
				b;
			return e;
		},
	...
	(g.initHtml4 = function() {
		...
		(g.initHtml4.initialized = !0), (g.enabled = !0), (g.savedHashes = [
			...
			return typeof a == "undefined"
				...
				: a < 0
						...
						: (b = g.savedHashes[a]), b;
		}), (g.discardedHashes = {}), (g.discardedStates = {
			...
			((g.hashChangeInit = function() {
				...
				return g.isInternetExplorer()
					? ((d = "historyjs-iframe"), (e = c.createElement(
							"iframe"
						)), e.setAttribute("id", d), (e.style.display =
							"none"), c.body.appendChild(
							e
						), e.contentWindow.document.open(), e.contentWindow.document.close(), (h =
							""), (i = !1), (g.checkerFunction = function() {
							if (i) return !1;
							i = !0;
							var c = g.getHash() || "",
								d =
							...
							return c !== b
								? ((b = c), d !== c &&
										((h = d = c), e.contentWindow.document.open(), e.contentWindow.document.close(), (e.contentWindow.document.location.hash = g.escapeHash(
											c
										))), g.Adapter.trigger(a, "hashchange"))
								: d !== h &&
										((h = d), g.setHash(d, !1)), (i = !1), !0;
						}))
			...
			((g.onHashChange = function(b) {
				...
				return g.isLastHash(e)
					? (g.busy(!1), !1)
					: (g.doubleCheckComplete(), g.saveHash(e), e &&
							g.isTraditionalAnchor(e)
							? (g.Adapter.trigger(a, "anchorchange"), g.busy(
									!1
								), !1)
							: ((f = g.extractState(
				...
				var h = g.createStateObject(b, d, e),
					...
					l = g.getHash();
				return g.storeState(h), (g.expectedStateId =
					h.id), g.recycleState(h), g.setTitle(h), i === k
					? (g.busy(!1), !1)
					: i !== l && i !== g.getShortUrl(c.location.href)
							? (g.setHash(i, !1), !1)
							: (g.saveState(h), g.Adapter.trigger(
									a,
									"statechange"
								), g.busy(!1), !0);
				...
				return g.discardState(f, e, h), g.pushState(
					e.data,
					e.title,
					e.url,
					!1
	...
	(m.init = function() {
		return typeof m.Adapter == "undefined"
			? !1
			: (typeof m.initCore != "undefined" &&
					m.initCore(), typeof m.initHtml4 != "undefined" &&
					m.initHtml4(), !0);
	}), (m.initCore = function() {
		if (typeof m.initCore.initialized != "undefined") return !1;
		(m.initCore.initialized = !0), (m.options = m.options || {
		}), (m.options.hashChangeInterval =
			m.options.hashChangeInterval ||
			100), (m.options.safariPollInterval =
			m.options.safariPollInterval ||
			500), (m.options.doubleCheckInterval =
			m.options.doubleCheckInterval || 500), (m.options.storeInterval =
			m.options.storeInterval || 1e3), (m.options.busyDelay =
			m.options.busyDelay || 250), (m.options.debug =
			m.options.debug || !1), (m.options.initialTitle =
			m.options.initialTitle || d.title), (m.intervalList = [
			...
			a
				? ((h = Array.prototype.slice.call(
						arguments
					)), (e = h.shift()), typeof c.debug != "undefined"
						? c.debug.apply(c, [e, h])
						: c.log.apply(c, [e, h]))
				: (e = "\n" + arguments[0] + "\n");
			...
			var a = (m.getInternetExplorerMajorVersion.cached = typeof m
				.getInternetExplorerMajorVersion.cached != "undefined"
			...
			var a = (m.isInternetExplorer.cached = typeof m.isInternetExplorer
				.cached != "undefined"
			...
			return (b = typeof b == "undefined" ? !0 : b), /[a-z]+\:\/\//.test(
				a
			) ||
				(d === "/"
			...
			return m.emulated.pushState &&
				(b = b.replace(c, "")), (b = b.replace(
				d,
				"/"
			)), m.isTraditionalAnchor(b) && (b = "./" + b), (b = b
				.replace(/^(\.\/)+/g, "./")
				.replace(/\#$/, "")), b;
		}), (m.store = {}), (m.idToState = m.idToState || {
		}), (m.stateToId = m.stateToId || {}), (m.urlToId = m.urlToId || {
		}), (m.storedStates = m.storedStates || [
		]), (m.savedStates = m.savedStates || [
		]), (m.normalizeStore = function() {
			(m.store.idToState = m.store.idToState || {}), (m.store.urlToId = m
				.store.urlToId || {}), (m.store.stateToId = m.store
				.stateToId || {});
			...
			(b = {}), (b.normalized = !0), (b.title =
				a.title || ""), (b.url = m.getFullUrl(
				m.unescapeString(a.url || d.location.href)
			)), (b.hash = m.getShortUrl(b.url)), (b.data = m.cloneObject(
				a.data
			)), (b.id = m.getIdByState(b)), (b.cleanUrl = b.url.replace(
				/\??\&_suid.*/,
				""
			)), (b.url = b.cleanUrl), (c = !m.isEmptyObject(b.data));
			if (b.title || c)
				(b.hash = m
					.getShortUrl(b.url)
					.replace(/\??\&_suid.*/, "")), /\?/.test(b.hash) ||
					(b.hash += "?"), (b.hash += "&_suid=" + b.id);
			return (b.hashedUrl = m.getFullUrl(b.hash)), (m.emulated
				.pushState ||
				m.bugs.safariPoll) &&
				m.hasUrlDuplicate(b) &&
				(b.url = b.hashedUrl), b;
			...
			var b, c, d;
			return (c = /(.*)\&_suid=([0-9]+)$/.exec(a)), (d = c
				? c[1] || a
				: a), (b = c ? String(c[2] || "") : ""), b || !1;
			...
			return (m.urlToId[a.url] = a.id), m.storedStates.push(
				m.cloneObject(a)
			), a;
		}), (m.isLastSavedState = function(a) {
			var b = !1, c, d, e;
			return m.savedStates.length &&
				((c = a.id), (d = m.getLastSavedState()), (e = d.id), (b =
					c === e)), b;
			...
			return b !== !1 && m.busy()
				...
				: ((c = m.escapeHash(a)), m.busy(!0), (e = m.extractState(
						a,
						!0
			...
			]), m.queues[a.queue || 0].push(a), m;
		}), (m.queue = function(a, b) {
			return typeof a == "function" && (a = { callback: a }), typeof b !=
				"undefined" && (a.queue = b), m.busy()
				? m.pushQueue(a)
				: m.fireQueueItem(a), m;
			...
			return (m.stateChanged = !1), m.doubleCheckClear(), m.bugs
				.ieDoubleCheck &&
			...
			return c || (c = m.createStateObject()), m.Adapter.trigger(
				a,
				"popstate"
			), m;
		}), (m.back = function(a) {
		...
		if (m.emulated.pushState) {
			var o = function() {};
			(m.pushState = m.pushState || o), (m.replaceState =
				m.replaceState || o);
		} else
			(m.onPopState = function(b, c) {
				...
				return m.doubleCheckComplete(), (g = m.getHash()), g
					? ((h = m.extractState(g || d.location.href, !0)), h
							...
							: (m.Adapter.trigger(a, "anchorchange"), m.busy(
									!1
								)), (m.expectedStateId = !1), !1)
					: ((e = m.Adapter.extractEventData("state", b, c) || !1), e
							? (f = m.getStateById(e))
							: m.expectedStateId
									? (f = m.getStateById(m.expectedStateId))
									: (f = m.extractState(d.location.href)), f ||
							(f = m.createStateObject(
								null,
								null,
								d.location.href
							)), (m.expectedStateId = !1), m.isLastSavedState(f)
							? (m.busy(!1), !1)
							: (m.storeState(f), m.saveState(f), m.setTitle(
									f
								), m.Adapter.trigger(a, "statechange"), m.busy(
									!1
								), !0));
			}), m.Adapter.bind(
				a,
				...
				return m.isLastSavedState(f)
					? m.busy(!1)
					: (m.storeState(f), (m.expectedStateId = f.id), n.pushState(
							f.id,
							f.title,
							f.url
						), m.Adapter.trigger(a, "popstate")), !0;
				...
				return m.isLastSavedState(f)
					? m.busy(!1)
					: (m.storeState(f), (m.expectedStateId =
							f.id), n.replaceState(
							f.id,
							f.title,
							f.url
		...
		m.Adapter.bind(a, "beforeunload", m.clearAllIntervals), m.Adapter.bind(
			...
			"unload",
			m.clearAllIntervals
		), m.saveState(m.storeState(m.extractState(d.location.href, !0))), f &&
			((m.onUnload = function() {
				var a, b;
				try {
				...
				(m.store = a), m.normalizeStore(), f.setItem(
					"History.store",
					k.stringify(a)
				);
			}), m.intervalList.push(
				i(m.onUnload, m.options.storeInterval)
			), m.Adapter.bind(a, "beforeunload", m.onUnload), m.Adapter.bind(
				a,
				"unload",
				m.onUnload
			));
